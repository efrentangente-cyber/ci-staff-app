...
class OfflineSyncManager {
    ...
    /**
     * Queues a completed interview for POST /api/ci/complete_interview when the device is offline.
     * Used by static/ci-checklist-wizard.js (same idempotent contract as online submit).
     */
    async saveChecklistOffline(applicationId, checklistData, signature, latitude, longitude) {
        if (typeof dbManager === 'undefined' || !dbManager.savePendingInterviewComplete) {
            throw new Error('Offline storage is not loaded — refresh the page.');
        }
        const checklistDataStr =
            typeof checklistData === 'string' ? checklistData : JSON.stringify(checklistData);
        let clientRequestId = '';
        try {
            if (typeof crypto !== 'undefined' && crypto.randomUUID) {
                ...
                clientRequestId =
                    'cli-' + Date.now().toString(36) + '-' + Math.random().toString(36).slice(2, 14);
            }
        } catch (e) {
            clientRequestId = 'cli-' + Date.now() + '-' + Math.random();
        }
        await dbManager.savePendingInterviewComplete({
            applicationId: applicationId,
            ciNotes: '',
            checklistDataStr: checklistDataStr,
            photoBlobs: [],
            clientRequestId: clientRequestId,
        });
        this.notifyListeners('checklist_queued_offline', { applicationId, clientRequestId });
